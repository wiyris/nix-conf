...
static const unsigned int borderspx =
    0; /* width of the border that start from outside the windows */
static const unsigned int borderepx =
    0; /* width of the border that start from inside the windows */
static const unsigned int borderspx_offset =
    0; /* offset of the border that start from outside the windows */
static const unsigned int borderepx_negative_offset =
    1; /* offset of the border that start from inside the windows */
...
static const float borderscolor[] =
    COLOR(0x @border
          @ff); /* color of the border that start from outside the windows */
static const float borderecolor[] = COLOR(
    0x @crust @ff); /* color of the border that start from inside the windows */
static const int border_color_type =
    BrdOriginal; /* borders to be colored (focuscolor, urgentcolor) */
...
static const int corner_radius_only_floating =
    1; /* only apply corner_radius and corner_radius_inner to floating windows
        */
...
const char *modes_labels[] = {
    "browser", "layout", "tui", "notification", "kb inhibit",
};
...
static const Rule rules[] = {
    /* app_id                    title       tags mask  isfloating  issticky
       monitor  x    y    width height  scratchkey */
    /* examples:
    { "Gimp",                    NULL,       0,         1,          0, -1,
    0, 	0,   500,  400,    0 },
    */
    // { "mpv",                     NULL,       0,         1,          0, -1, 0,
    // 0,   0.8f, 0.8f, 0 }, { "imv",                     NULL,       0, 1, 0,
    // -1,      0,   0,   0,    0,      0 },
    ...
    // { "^foot-.*",                NULL,       1 << 1,    0,          0, -1, 0,
    // 0,   0,    0,      0 }, { "^footclient-.*",          NULL,       1 << 1,
    // 0,          0,        -1,      0,   0,   0,    0,      0 }, {
    // "^kitty-.*",               NULL,       1 << 1,    0,          0, -1, 0,
    // 0,   0,    0,      0 },
...
static const MonitorRule monrules[] = {
    ...
    -1,  -1 },
    */
    // { "DP-1",     0.5, 1,      1,    &layouts[0], WL_OUTPUT_TRANSFORM_NORMAL,
    // -1,  -1 }, { "eDP-1",    0.5, 1,      2,    &layouts[0],
...
#define SHCMD_UWSM(cmd)                                                        \
  {                                                                            \
    .v = (const char *[]) { "sh", "-c", "uwsm-app -- " cmd, NULL }             \
  }

// same as SHCMD_UWSM but adds scratchkey
#define SHCMD_UWSM_SK(scratchkey, cmd)                                         \
  {                                                                            \
    .v = (const char *[]) { scratchkey, "sh", "-c", "uwsm-app -- " cmd, NULL } \
  }
...
static const Key keys[] = {
    ...
    {MODKEY, Key_i, raiserunnamedscratchpad,
     SHCMD_UWSM_SK("i", "~/.config/kitty/scripts/kitty-scratch.sh")},
    ...
    // Open file manager
    {MODKEY | MOD_ALT, Key_f, raiserunnamedscratchpad,
     SHCMD_UWSM_SK("f", "Thunar --name Thunar")},
    ...
    {MOD_NONE, Key_XF86AudioRaiseVolume, spawn,
     SHCMD("progress-osd --volume +5%")},
    {MOD_NONE, Key_XF86AudioLowerVolume, spawn,
     SHCMD("progress-osd --volume -5%")},
    ...
    {MOD_NONE, Key_XF86MonBrightnessUp, spawn,
     SHCMD("progress-osd --backlight 5%+")},
    {MOD_NONE, Key_XF86MonBrightnessDown, spawn,
     SHCMD("progress-osd --backlight 5%-")},

    {MOD_NONE, Key_XF86KbdBrightnessUp, spawn,
     SHCMD("progress-osd --kbd-backlight 5%+")},
    {MOD_NONE, Key_XF86KbdBrightnessDown, spawn,
     SHCMD("progress-osd --kbd-backlight 5%-")},
    ...
    {MODKEY | MOD_ALT, Key_x, raiserunnamedscratchpad,
     SHCMD_UWSM_SK("x", "xwaylandvideobridge")},
...
#define CHVT(KEY, n)                                                           \
  {                                                                            \
    MOD_CONTROL | MOD_ALT, KEY, chvt, { .ui = (n) }                            \
  }
    CHVT(Key_F1, 1),
    CHVT(Key_F2, 2),
    CHVT(Key_F3, 3),
...
#define EXIT_TO_NORMAL_MODE(mode_index, mod, keycode, func, arg)               \
  {mode_index, {mod, keycode, func, arg}}, {                                   \
    mode_index, {                                                              \
      mod, keycode, entermode, { .i = NORMAL }                                 \
    }                                                                          \
  }
    ...
    EXIT_TO_NORMAL_MODE(LAYOUT, MOD_NONE, Key_t, setlayout,
                        {.v = &layouts[LAYOUT_TILE]}),
    EXIT_TO_NORMAL_MODE(LAYOUT, MOD_NONE, Key_f, setlayout,
                        {.v = &layouts[LAYOUT_BSTACK]}),
    EXIT_TO_NORMAL_MODE(LAYOUT, MOD_SHIFT, Key_f, setlayout,
                        {.v = &layouts[LAYOUT_FLOAT]}),
    EXIT_TO_NORMAL_MODE(LAYOUT, MOD_NONE, Key_m, setlayout,
                        {.v = &layouts[LAYOUT_MONOCLE]}),
    ...
    EXIT_TO_NORMAL_MODE(BROWSER, MOD_NONE, Key_g, spawn,
                        SHCMD_UWSM("google-chrome-stable")),
    EXIT_TO_NORMAL_MODE(BROWSER, MOD_NONE, Key_m, spawn,
                        SHCMD_UWSM("microsoft-edge")),
    EXIT_TO_NORMAL_MODE(
        BROWSER, MOD_SHIFT, Key_i, raiserunnamedscratchpad,
        SHCMD_UWSM_SK("g", "microsoft-edge --profile-directory=Default "
                           "--app=https://www.bing.com/chat")),
    EXIT_TO_NORMAL_MODE(
        BROWSER, MOD_NONE, Key_i, raiserunnamedscratchpad,
        SHCMD_UWSM_SK("c", "google-chrome-stable --profile-directory=Default "
                           "--app=https://chat.openai.com")),
    EXIT_TO_NORMAL_MODE(
        BROWSER, MOD_NONE, Key_o, raiserunnamedscratchpad,
        SHCMD_UWSM_SK("o", "google-chrome-stable --profile-directory=Default "
                           "--app=https://ollama.wochap.local")),
    EXIT_TO_NORMAL_MODE(
        BROWSER, MOD_NONE, Key_w, raiserunnamedscratchpad,
        SHCMD_UWSM_SK("w", "google-chrome-stable --profile-directory=Default "
                           "--app=https://openwebui.wochap.local")),
    EXIT_TO_NORMAL_MODE(
        BROWSER, MOD_NONE, Key_u, raiserunnamedscratchpad,
        SHCMD_UWSM_SK("y", "google-chrome-stable --profile-directory=Default "
                           "--app=https://music.youtube.com")),
    {BROWSER, {MOD_NONE, Key_Escape, entermode, {.i = NORMAL}}},

    // Terminal TUI
    EXIT_TO_NORMAL_MODE(TUI, MOD_NONE, Key_n, raiserunnamedscratchpad,
                        SHCMD_UWSM_SK("n", "tui-notes")),
    EXIT_TO_NORMAL_MODE(TUI, MOD_NONE, Key_i, raiserunnamedscratchpad,
                        SHCMD_UWSM_SK("b", "tui-notes-obsidian")),
    EXIT_TO_NORMAL_MODE(TUI, MOD_NONE, Key_m, raiserunnamedscratchpad,
                        SHCMD_UWSM_SK("m", "tui-monitor")),
    EXIT_TO_NORMAL_MODE(TUI, MOD_NONE, Key_e, raiserunnamedscratchpad,
                        SHCMD_UWSM_SK("e", "tui-email")),
    EXIT_TO_NORMAL_MODE(TUI, MOD_NONE, Key_r, raiserunnamedscratchpad,
                        SHCMD_UWSM_SK("r", "tui-rss")),
    EXIT_TO_NORMAL_MODE(TUI, MOD_NONE, Key_u, raiserunnamedscratchpad,
                        SHCMD_UWSM_SK("u", "tui-music")),
    EXIT_TO_NORMAL_MODE(TUI, MOD_NONE, Key_d, raiserunnamedscratchpad,
                        SHCMD_UWSM_SK("d", "tui-notification-center")),
    EXIT_TO_NORMAL_MODE(TUI, MOD_NONE, Key_c, raiserunnamedscratchpad,
                        SHCMD_UWSM_SK("k", "tui-calendar")),
    EXIT_TO_NORMAL_MODE(TUI, MOD_NONE, Key_b, spawn,
                        SHCMD_UWSM("tui-bookmarks --select")),
    EXIT_TO_NORMAL_MODE(TUI, MOD_SHIFT, Key_b, spawn,
                        SHCMD_UWSM("tui-bookmarks --add")),
    EXIT_TO_NORMAL_MODE(TUI, MOD_CONTROL | MOD_SHIFT, Key_b, spawn,
                        SHCMD_UWSM("tui-bookmarks --edit")),
    {TUI, {MOD_NONE, Key_Escape, entermode, {.i = NORMAL}}},

    // Notification
    // Show recent notification menu
    EXIT_TO_NORMAL_MODE(NOTIFICATION, MOD_NONE, Key_a, spawn,
                        SHCMD("dunstctl context")),
    // Show last notification
    EXIT_TO_NORMAL_MODE(NOTIFICATION, MOD_NONE, Key_h, spawn,
                        SHCMD("dunstctl history-pop")),
    // Hide recent notification
    EXIT_TO_NORMAL_MODE(NOTIFICATION, MOD_NONE, Key_c, spawn,
                        SHCMD("dunstctl close")),
    // Hide all notifications
    EXIT_TO_NORMAL_MODE(NOTIFICATION, MOD_SHIFT, Key_c, spawn,
                        SHCMD("dunstctl close-all")),
    {NOTIFICATION, {MOD_NONE, Key_Escape, entermode, {.i = NORMAL}}},

    // HACK: disable all dwl keymappings
    {KB_INHIBIT,
     {MODKEY | MOD_ALT | MOD_CONTROL, Key_g, entermode, {.i = NORMAL}}},
};
